...
PlayerSort.py then sorts files out using PlayerID as this is the unique identifier and primary key in separating each players respective data, each time this script is run it will update player files accordingly as well as account for any new players. 

This script also further cleans by removing any tests that are sequence "4" or higher. 
  Why? 
    Tests have show that after four or so trials metric reliability is becomes slightly more questionable due to athlete fatigue. 
